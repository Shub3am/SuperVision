...
export default function Home(): React.FCcomponent {
  ...
  <div className="flex flex-wrap sm:flex-nowrap items-center justify-center">
    ...
    " onChange={e=> setFile(e.target.files[0])} disabled={true}/>
    </div>
    <div>
      
      <button style={{width: "11em"}} onClick={(e)=> { e.preventDefault()
...
<span className="relative text-white">SpeedRead PDF Work In Progress</span>
</button>
</div>
  </div>
...
<textarea placeholder="Get Started Now" className='text-black p-5 m-5 w-5/6 lg:w-2/3'  onChange={(e)=> setText(e.target.value)} id="normalText" name="normalText" rows="10" cols="50"/>
  ...
  <div className="p-6" style={{height: "500px", overflowY: "scroll"}}>
    ...
    <h5
      className="mb-1 text-xl font-medium leading-2 text-neutral-800 dark:text-neutral-50" style={{letterSpacing: 0, lineHeight: "40px", color: "#c5bbbb"}} dangerouslySetInnerHTML={{__html: speedReadText}}>
        ...
        <div className="py-5 mt-5 mx-auto text-center md:max-w-screen-md lg:max-w-screen-lg lg:px-36">
            <span className="font-semibold text-gray-400 uppercase">Showcased In</span>
            <div className="flex flex-wrap justify-center items-center mt-8 text-gray-500 sm:justify-center">
                <a href="#" className="mr-5 mb-5 lg:mb-0 hover:text-gray-800 dark:hover:text-gray-400">
                    <Image height={300} src="/HTM.webp" alt="hackthemountains" width={300}/>
                </a>         
            </div>
        </div> 
    </div>
